...
class RedmrtScraper:
    ...
    def norm_url(self, url: str) -> str:
        ...
        # ВАЖНО: на redmrt.ru товар часто открывается как
        # /kategoriya/podkategoriya?product_id=6273.
        # Поэтому product_id нельзя удалять из query, иначе ссылка превращается
        # обратно в страницу категории и характеристики товара не находятся.
        keep = []
        for k, v in parse_qsl(p.query, keep_blank_values=True):
            if k.lower() in {"product_id", "path"}:
                keep.append((k, v))
    ...
    def category_product_links(self, category_url: str) -> Set[str]:
        ...
        for sel in selectors:
            for a in soup.select(sel):
                ...
                has_product_id = any(k.lower() == "product_id" and v for k, v in parse_qsl(parsed.query))

                # На сайте товары часто имеют URL вида /category/subcategory?product_id=1234.
                # Также оставляем старый вариант, если товар лежит глубже категории.
                if text.lower() not in SKIP_TEXT and (has_product_id or href.startswith(category_url + "/")):
                    links.add(href)
        return links
    ...
    def extract_characteristics(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        # 2) Строки характеристик в div/li: название слева, значение справа, между ними точки/лидеры.
        # На redmrt.ru блок часто выглядит не как table, а как набор строк внутри вкладки "Характеристики".
        candidate_roots = []
        for text_node in soup.find_all(string=re.compile(r"Технические характеристики|Характеристики", re.I)):
            parent = text_node.parent
            for root in [parent, parent.parent if parent else None, parent.parent.parent if parent and parent.parent else None]:
                if isinstance(root, Tag) and root not in candidate_roots:
                    candidate_roots.append(root)
        candidate_roots.extend(soup.select("#tab-specification, #tab-characteristic, #tab-characteristics, [id*=character], [class*=character], [class*=specif]"))

        for root in candidate_roots:
            for row in root.find_all(["li", "tr", "div", "p"], recursive=True):
                # Берём только локальный текст строки, без огромных вложенных контейнеров.
                children_texts = [self.clean(ch.get_text(" ")) for ch in row.find_all(recursive=False) if isinstance(ch, Tag)]
                children_texts = [x for x in children_texts if x and x.lower() not in SKIP_TEXT]
                if len(children_texts) >= 2:
                    k, v = children_texts[0], children_texts[-1]
                    if (k and v and k != v and len(k) <= 80 and len(v) <= 200
                            and not PRICE_RE.search(v) and not STOCK_RE.search(k)
                            and not re.search(r"описание|отзывы|вопросы", k, re.I)):
                        specs[k.rstrip(".:")] = v
                        continue
                ...
                if "|" in txt:
                    ...
                    # Удаляем повторяющиеся куски от вложенных элементов.
                    compact = []
                    for part in parts:
                        if part not in compact and part.lower() not in SKIP_TEXT:
                            compact.append(part)
                    if len(compact) >= 2:
                        k, v = compact[0], compact[-1]
                        if (k and v and k != v and len(k) <= 80 and len(v) <= 200
                                and not PRICE_RE.search(v) and not STOCK_RE.search(k)
                                and not re.search(r"описание|отзывы|вопросы", k, re.I)):
                            specs[k.rstrip(".:")] = v
        ...
        for dl in soup.find_all("dl"):
            ...
            for dt, dd in zip(dts, dds):
                ...
                if k and v and not PRICE_RE.search(v) and not STOCK_RE.search(k):
                    specs[k] = v
        # 4) Fallback для страниц, где web-текст показывает характеристики строками.
